...
export default function Home() {
    ...
    const [filters, setFilters] = useState<SidebarFilters | undefined>(
        undefined,
    );
    ...
    useEffect(() => {
        ...
        // Fetch both external and internal jobs in parallel
        Promise.all([
            fetch(`${API_URL}/api/jobs`).then((res) => res.json()),
            fetch(`${API_URL}/api/jobs/internal`).then((res) => res.json()),
        ])
            .then(([externalJobs, internalJobs]) => {
                console.log("External jobs fetched:", externalJobs.length);
                console.log("Internal jobs fetched:", internalJobs.length);

                // Merge both job arrays
                    ...
                    <div className="flex gap-25 py-10 px-15">
                        ...
                        <div className="flex-1 font-sans flex flex-col gap-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-10">
                                {(filters || searchQuery ? filteredJobs : jobs)
                                    .length === 0 && (
                                    <div className="col-span-full">
                                        <p className="text-[#0090D9] text-lg">
                                            No jobs found.
                                        </p>
                                    </div>
                                )}
