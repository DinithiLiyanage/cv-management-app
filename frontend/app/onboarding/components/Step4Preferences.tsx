        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            {[
                ...
                icon: (
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M12 19v-2m0 0a7 7 0 100-14 7 7 0 000 14zm0 0c3.866 0 7 3.134 7 7H5c0-3.866 3.134-7 7-7z"
                    />
              ...
              <label
                key={option.id}
                className={`relative p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 flex flex-col items-center ${
                  data.workLocation === option.id
                    ? "border-[#0090D9] bg-blue-50"
                    : "border-gray-200 hover:border-[#0090D9]"
                }`}
              >
                <input
                  ...
                  name="workLocation"
                  value={option.id}
                  checked={data.workLocation === option.id}
                  onChange={() => updateData({ workLocation: option.id })}
                  className="sr-only"
                />
                <div
                  className={`w-12 h-12 ${option.bg} rounded-full flex items-center justify-center mx-auto mb-2`}
                >
                  {option.icon}
                </div>
                <h4 className="text-sm font-medium text-gray-800">
                ...
                {data.workLocation === option.id && (
                  <span className="absolute top-2 right-2 w-5 h-5 bg-[#0090D9] rounded-full text-white text-xs flex items-center justify-center">
                    ✓
                  </span>
                )}
              </label>
